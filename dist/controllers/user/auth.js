        ...
        if (userDetails) {
            if (userDetails?.isActive) {
                ...
                if (await argon2_1.default.verify(userDetails.password1, password + "" + process.env.Password_Secret_Key)) {
                    ...
                    await (0, helpers_1.offline_online_all_rooms)({ userId: userDetails._id, status: 'online', name: userDetails.name });
...
const dd = async () => {
    await index_2.chat_room_messageModel.deleteMany({ messageType: { $in: ['online', 'offline'] } });
};
// dd();
const userInfo = async (req, res, next) => {
    try {
        const { countryCode, phoneNumber } = req.query;
        const userDetails = await index_2.userModel.findOne({ countryCode: "+" + countryCode, phoneNumber: phoneNumber, isDelete: false }, { countryCode: 1, name: 1, image: 1, phoneNumber: 1 });
        if (userDetails) {
...
const logout = async (req, res, next) => {
    try {
        ...
        if (user_sessionDetails) {
            ...
            const user_details = await index_2.userModel.findOneAndUpdate({ _id: user_sessionDetails.userId }, { onlineStatus: false });
            await (0, helpers_1.offline_online_all_rooms)({ userId: user_sessionDetails.userId, status: 'offline', name: user_details?.name ?? '' });
