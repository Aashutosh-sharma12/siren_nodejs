            ...
            if (data.code == 200 && data.data.count > 0) {
                                ...
                                if (data.data.code == 200) {
                                    ...
                                    for (let item of itemList) {
                                        ...
                                        document.getElementById('table').innerHTML += '<tr >' +
                                            ...
                                            '<button type="button" class="btn btn-sm btn-info" style="margin: 5px;"  data-toggle="modal" data-target="#myModal2" onclick= View(' + '\'' + userId + '\'' + ')>' + 'Edit' + '</button>' +
                                            // diable_button +
                ...
                for (let item of itemList) {
                    ...
                    document.getElementById('table').innerHTML += '<tr >' +
                        ...
                        '<td><span class="label label-primary">' + y +
                        '<td> <button type="button" class="btn btn-sm btn-success" id="Action_button" style="margin: 5px;"  data-target="' + status + '" onclick= "updateStatus(' + '\'' + userId + '\'' + ')">' + x + '</button>' +
                        '<button type="button" class="btn btn-sm btn-info" style="margin: 5px;"  data-toggle="modal" data-target="#myModal2" onclick= details(' + '\'' + userId + '\'' + ')>' + 'Edit' + '</button>' +
                        // diable_button +
                        '<button type="button" class="btn btn-sm btn-warning" id="Action_button1" style="margin: 5px;" onclick= deleteSub(' + '\'' + userId + '\'' + ')>' + 'Delete' + '</button>' +
                        '</tr>'
                }
...
function details(subId) {
    $.ajax({
        ...
        document.getElementById('uniqueId').value = data.data.uniqueId ? data.data.uniqueId : "N/A";
        populateEditFeatures(Array.isArray(data.data.features) ? data.data.features : []);
        $('#myModal2').modal('show');
    }).fail(function (jqXHR, textStatus, errorThrown) {
...
function submitEditSubscription() {
    const data = {
        ...
        subscriptionType: document.getElementById("sub_type").value,
        amount: document.getElementById("amount").value,
        subId: document.getElementById("uniqueId").value,
        features: []
    };
    const featureList = document.getElementById("featureList").children;
    for (let i = 0; i < featureList.length; i++) {
        let text = featureList[i].textContent.trim();
    ...
    $.ajax({
        type: "put",
        url: `${host}/api/v1/admin/sub/edit`,
        contentType: "application/json",
        dataType: 'json',
...
function addSubscription() {
    const data = {
        ...
        subscriptionType: document.getElementById("sub_type1").value,
        amount: document.getElementById("amount1").value,
        features: []
    };
    const featureList = document.getElementById("featureList1").children;
    for (let i = 0; i < featureList.length; i++) {
        let text = featureList[i].textContent.trim();
        text = text.replace(/^\d+\.\s*/, '').replace(/🗑/, '').trim();
        data.features.push(text);
