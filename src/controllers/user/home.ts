...
const contactList = async (req: any, res: Response, next: NextFunction) => {
    try {
        const { id } = req.user;
            ...
            pipeLine.push(
                {
                    $addFields: {
                        ...
                        participants_exists_message: messages.already_exist_in_group
                    }
                },
                {
                    $lookup: {
...
const all_room_list = async (req: any, res: Response, next: NextFunction) => {
    try {
        const { id } = req.user;
        const { page = 1, perPage = 10, search, isGroup = false } = req.query;
        const list = await chat_room_participantModel.aggregate([
...
const feed_list = async (req: any, res: Response, next: NextFunction) => {
    try {
        const { id } = req.user;
        ...
        const [list, count] = await Promise.all([feedModel.aggregate([
            {
                $match: cond
            },
            {
